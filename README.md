...
- outside DP（inside-outside）：以纯整数下发“外部补全方式数”统计每条候选弧
  出现在多少个最优方案中（不做份额除法，避免 2^53 量级浮点舍入），据此分类
...
1. 代码测试（82 项，含 60 组随机输入与暴力枚举的方案数/规范解/分类对照，以及
   171 击中 / 226 候选、4^28+1 个最优方案的大规模同优一致性测试）；
2. 构建检查（语法编译、模块导入、镜像内关键文件齐备）；
3. API/HTTP 冒烟（健康路径、嵌套同优、交叉低价诱饵、空候选、非法引用、
   重复端点对、位置冲突、规模越界、大规模同优、未知路径）。
